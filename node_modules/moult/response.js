...
let stream = fs.createReadStream(path.join(env, file));
stream.pipe(this);
...
fs.readFile(
path.join(
env,app.Sets.views,type,'header.html'),
(err,data) => {
...
env,app.Sets.views,type,page+'.html'),
(err,data) => {
if(err) { this.end(); return 0; }
stem.parse(data.toString(),opt)
.then((temp) => {
this.write(temp);
fs.readFile(path.join(
env,app.Sets.views,type,'footer.html'),
(err,data) => {
...
this.is('html');
fs.readFile(path.join(env,app.Sets.views,file+'.html'),
(err,cont) => {
if(err) { return this.end(''); }
